...
use_dict={}
use_dict['remote']='\\\\MACBOOKPRO-AE22\\Tema2'
...
def createFiles():
	...
	def createFile():
		print "Creating files1"
		seed = "1092384956781341341234656953214543219"
		seedLength = len(seed) - 1
		words = open("//MACBOOKPRO-AE22/Tema2/lorem.txt", "r").read().replace("\n", '').split()

		def fdata():
		
			print "Creating files2"
...
def makeResultsFile(results):
	filePath = os.path.join(sharedFolderPath, resultsPrefix + username)
...
sharedFolderPath = "//MACBOOKPRO-AE22/Tema2/"
allWorkDoneFile = sharedFolderPath + "\\" + allWorkDoneFilename
remainingTasksFile = sharedFolderPath + "\\" + remainingTasksFilename
allResultsFile = "//MACBOOKPRO-AE22/Tema2/" + allResultsFilename

print "Startign program"
createFiles()
start()
